...
import com.hikone.artisanworktables.common.tile.handler.ToolboxItemStackHandler;
...
public class ToolboxBlockEntity extends BlockEntityDataBase
{
    ...
    {
        super(ArtisanWorktablesMod.TileEntityTypes.TOOLBOX, pos, state, ArtisanWorktablesMod.getProxy().getTileDataService());

        Predicate<ItemStack> predicate = itemStack -> itemStack.isEmpty() || this.allowNonToolItems() || ToolValidationHelper.isValidTool(itemStack, ArtisanWorktablesMod.getProxy().getRecipeManager());
        ...
        this.registerTileDataForNetwork(new ITileData[]{new TileDataItemStackHandler<>(this.itemStackHandler)});
    }
    ...
    {
        return this.level != null && this.level.getBlockState(this.getBlockPos()).getBlock() instanceof ToolboxMechanicalBlock;
    }
    ...
    {
        return this.level != null && this.level.getBlockEntity(this.getBlockPos()) == this && player.distanceToSqr(this.getBlockPos().getX() + 0.5, this.getBlockPos().getY() + 0.5, this.getBlockPos().getZ() + 0.5) <= 64;
    }
