...
public class TileDataTracker
{
    ...
    private RegistryFriendlyByteBuf packetBuffer;
    ...
    /* package */ FriendlyByteBuf getUpdateBuffer()
    {
        ...
        {
            this.packetBuffer = new RegistryFriendlyByteBuf(Unpooled.buffer(), this.tile.getLevel().registryAccess());
        }
    ...
    /* package */ void updateClient(FriendlyByteBuf buffer) throws IOException
    {
        ...
        if (dirtyCount > 0)
        {
            ...
            for (int i = 0; i < dirtyCount; i++)
            {
                ...
                TileDataServiceClientMonitor.onClientTrackerUpdateReceived(this.tile.getBlockPos(), data.getClass());
            }
