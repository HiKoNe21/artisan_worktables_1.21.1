...
public class ItemRegistrationEventHandler
{
    ...
    public void on(RegisterEvent event)
    {
        ...
        this.registerBlockItem(event, ArtisanWorktablesMod.Blocks.TOOLBOX);
        this.registerBlockItem(event, ArtisanWorktablesMod.Blocks.MECHANICAL_TOOLBOX);
    }
    ...
    private void registerBlockItem(RegisterEvent event, Block block)
    {
        ...
        if (blockId != null)
        {
            event.register(Registries.ITEM, blockId, () -> new BlockItem(block, new Item.Properties()));
        }
    }
}
