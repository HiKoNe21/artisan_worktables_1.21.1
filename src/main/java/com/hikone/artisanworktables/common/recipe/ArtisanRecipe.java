...
public abstract class ArtisanRecipe implements Recipe<ArtisanInventory>
{
    ...
    @Nonnull
    @Override
    public ItemStack assemble(@Nonnull ArtisanInventory inventory, @Nonnull HolderLookup.Provider registries)
    {
        return this.getResultItem();
    }
    ...
    public boolean matches(ArtisanInventory inventory, Level world)
    {
        ...
        if (inventory.getPlayerData().isPresent() && !this.matchPlayer(inventory.getPlayerData().get()))
        {
            return false;
        }
        ...
        if (!this.secondaryIngredients.isEmpty() && !inventory.getSecondaryIngredientMatcher().matches(this.secondaryIngredients))
        {
            return false;
        }
    ...
    private boolean matchTools(ItemStack[] tools, IToolHandler[] toolHandlers)
    {
        ...
        for (int i = 0; i < tools.length; i++)
        {
            for (int j = 0; j < toolCount; j++)
            {
                ...
                if ((mask & bit) != bit && this.tools.get(j).matches(toolHandlers[i], tools[i]) && this.hasSufficientToolDurability(toolHandlers[i], tools[i]))
                {
                    mask |= bit;
                    matchCount += 1;
                    continue tableTools;
