...
import com.hikone.artisanworktables.common.util.Util;
...
public class CraftHandler
{
            ...
            List<ItemStack> output)
    {
        ...
        if (!world.isClientSide && !craftedItem.isEmpty())
        {
    ...
    private void onCraftReduceExperience(@Nullable Player player, boolean consumeExperience, int experienceRequired, int levelRequired)
    {
        ...
        if (consumeExperience)
        {
            ...
            {
                int experience = EnchantmentHelper.getExperienceToLevel(player.experienceLevel - levelRequired, player.experienceLevel);
    ...
    private void onCraftReduceIngredients(ArtisanInventory inventory)
    {
        ...
        List<ItemStack> remainingItems = this.getRemainingItems(matrixHandler, NonNullList.withSize(matrixHandler.getSlots(), ItemStack.EMPTY));
     ...
     */
    @Nonnull
    private List<ItemStack> getRemainingItems(ICraftingMatrixStackHandler matrixHandler, NonNullList<ItemStack> itemStacks)
    {
    ...
    {
        if (!consumeSecondaryIngredients || secondaryIngredientHandler == null)
        {
            return;
        }
    ...
    private ItemStack getRemainingSecondaryItem(IItemHandlerModifiable secondaryIngredientHandler, Ingredient ingredient, ItemStack stack)
    {
    ...
    private void damageTools(ArtisanRecipe recipe, IItemHandlerModifiable toolStackHandler, NonNullList<ToolEntry> tools,
                             Level world, @Nullable Player player, BlockPos position, @Nullable IItemHandler toolReplacementHandler)
    {
        ...
        { // recipe tools
            ...
            for (int j = 0; j < toolStackHandler.getSlots(); j++)
            { // table tools
                int bit = 1 << j;
                ...
                { // slot isn't empty
                    ...
                    if (tool.matches(toolHandler, stackInSlot) // tool matches recipe
                            && recipe.hasSufficientToolDurability(toolHandler, stackInSlot))
                    { // tool has durability
                        ...
                        if (broken)
                        {
                            if (!world.isClientSide)
                            {
                                world.playSound(null, position.getX(), position.getY(), position.getZ(), SoundEvents.ITEM_BREAK, SoundSource.PLAYERS, 1.0f, 1.0f);
                            }
                        }
    ...
    private boolean onCraftDamageTool(ArtisanRecipe recipe, ItemStack itemStack, Level world, @Nullable Player player)
    {
        if (!itemStack.isEmpty())
        {
            ...
            return toolDamage > 0 && toolHandler.applyDamage(world, itemStack, toolDamage, player, false);
        }
    ...
    private void onCraftCheckAndReplaceTool(ArtisanRecipe recipe, int toolIndex, ItemStack toolCopy, IItemHandlerModifiable toolStackHandler, IItemHandler capability)
    {
        ...
        if (!recipe.hasSufficientToolDurability(toolHandler, itemStack))
        {
            ...
            for (int i = 0; i < slotCount; i++)
            {
                ...
                if (toolEntry != null && toolEntry.matches(toolHandler, toolCopy) && recipe.hasSufficientToolDurability(potentialToolHandler, potentialTool))
                {
    ...
    protected void onCraftCompleteServer(ItemStack craftedItem, ArtisanInventory inventory, @Nullable Player player, BlockPos pos)
    {
        if (player == null)
        {
            return;
        ...
        NeoForge.EVENT_BUS.post(new PlayerEvent.ItemCraftedEvent(player, craftedItem.copy(), inventory));
