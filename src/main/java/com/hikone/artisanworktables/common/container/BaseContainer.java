...
import com.hikone.artisanworktables.common.container.slot.*;
import com.hikone.artisanworktables.common.network.SCPacketWorktableContainerJoinedBlockBreak;
...
import com.hikone.artisanworktables.common.tile.*;
import com.hikone.artisanworktables.common.util.CraftSoundHelper;
...
public abstract class BaseContainer extends ContainerBase
{
    ...
    public BaseContainer(@Nullable MenuType<?> type, int id, Level world, BlockPos pos, Inventory playerInventory, double mouseX, double mouseY)
    {
        ...
        this.craftingResultSlot = new CraftingResultSlot(this.tile, this::updateRecipeOutput, this.resultHandler, 0, this.getResultPositionX(), this.getResultPositionY());
        ...
        for (int y = 0; y < craftingMatrixHandler.getHeight(); ++y)
        {
            ...
            {
                this.containerSlotAdd(new CraftingIngredientSlot(slotChangeListener, craftingMatrixHandler, x + y * craftingMatrixHandler.getWidth(), 20 + x * 18, 17 + y * 18));
            }
        }
...

        {
            ...
            {
                this.containerSlotAdd(new CraftingToolSlot(slotChangeListener, itemStack -> ToolValidationHelper.isValidTool(this.tile.getTableType(), itemStack, world.getRecipeManager()), toolHandler, i, 78 + this.getToolOffsetX(), 35 + 22 * i + this.getToolOffsetY()));
            }
        ...
        if (this.tile instanceof WorkshopBlockEntity)
        {
            for (int i = 0; i < 3; i++)
            {
                this.containerSlotAdd(new CraftingExtraResultSlot(this.tile.getSecondaryOutputHandler(), i, 116 + i * 18, 17));
            }
        }
        else
        {
            for (int i = 0; i < 3; i++)
            {
                this.containerSlotAdd(new CraftingExtraResultSlot(this.tile.getSecondaryOutputHandler(), i, 152, 17 + i * 18));
            }
        }
        ...
        if (this.tile instanceof SecondaryInputBaseBlockEntity)
        {
            ...
            {
                this.containerSlotAdd(new CraftingSecondarySlot(slotChangeListener, handler, i, 8 + i * 18, 75 + this.getSecondaryInputOffsetY()));
            }
        ...
        if (this.canPlayerUseToolbox())
        {
            ...
            for (int x = 0; x < 3; x++)
            {
                for (int y = 0; y < 9; y++)
                {
                    ToolboxSideSlot toolboxSideSlot = new ToolboxSideSlot(this::canPlayerUseToolbox, itemHandler, y + x * 9, x * -18 + this.containerToolboxOffsetGetX(), y * 18 + 8);
    ...
    public void onJoinedBlockBreak(Level world, BlockPos pos)
    {
        ...
        {
            ArtisanWorktablesMod.getProxy().getPacketService().sendToPlayer((ServerPlayer) this.player, new SCPacketWorktableContainerJoinedBlockBreak(pos));
        }
    }
    ...
    private boolean swapItemStack(int originSlotIndex, int targetSlotIndex, boolean swapOnlyEmtpyTarget)
    {
        ...
        if (!originStack.isEmpty() && targetSlot.mayPlace(originStack))
        {
            if (targetStack.isEmpty())
            {
    ...
    public void clicked(int slotId, int dragType, @Nonnull ClickType clickType, @Nonnull Player player)
    {
        ...
        if (slotId == this.slotIndexResult || (slotId >= this.slotIndexSecondaryOutputStart && slotId <= this.slotIndexSecondaryOutputEnd))
        {
        ...
        if (resultInteraction && recipe != null && this.world.isClientSide && this.tile.getCraftCounter() != craftCountBefore)
        {
    ...
    public void broadcastChanges()
    {
        ...
        if (this.tile == null || world == null || world.isClientSide)
        {
            return;
        }
        ...
        if (this.lastFluidStack != FluidStack.EMPTY && fluidStack == FluidStack.EMPTY)
        {
            this.lastFluidStack = null;
            this.updateRecipeOutput();

        }
        else if (this.lastFluidStack == FluidStack.EMPTY && fluidStack != FluidStack.EMPTY)
        {
        ...
        {
            if (!FluidStack.matches(lastFluidStack, fluidStack))
            {
                this.lastFluidStack = fluidStack.copy();
                this.updateRecipeOutput();
            }
