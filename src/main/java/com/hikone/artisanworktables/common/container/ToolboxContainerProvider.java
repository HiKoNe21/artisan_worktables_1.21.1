...
import com.hikone.artisanworktables.common.block.ToolboxBaseBlock;
...
public class ToolboxContainerProvider implements MenuProvider
{
    ...
    {
        return Component.translatable("block." + ArtisanWorktablesMod.MODID + "." + BuiltInRegistries.BLOCK.getKey(this.block).getPath());
    }
    ...
    public AbstractContainerMenu createMenu(int id, @Nonnull Inventory playerInventory, @Nonnull Player playerEntity)
    {
        ...
        {
            return new ToolboxContainer(id, this.world, this.pos, playerInventory);
        }
        else if (this.block instanceof ToolboxMechanicalBlock)
        {
            return new ToolboxMechanicalContainer(id, this.world, this.pos, playerInventory);
        }
        else
        {
