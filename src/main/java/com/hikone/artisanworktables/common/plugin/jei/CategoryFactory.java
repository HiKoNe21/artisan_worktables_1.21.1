...
{
    public BaseCategory<?> create(EnumTier tier, EnumType type, Block block, IGuiHelper guiHelper, CategorySetupHandler categorySetupHandler, CategoryDrawHandler categoryDrawHandler)
    {
        return new Category(tier, type, String.format("block.%s.%s_%s", ArtisanWorktablesMod.MODID, tier.getName(), type.getName()), this.createBackground(tier, guiHelper, Key.from(String.format("textures/gui/%s_%s.png", tier.getName(), type.getName()))), guiHelper.createDrawableItemStack(new ItemStack(block)), Plugin.CATEGORY_KEYS.get(tier).get(type), categorySetupHandler, categoryDrawHandler);
    }
