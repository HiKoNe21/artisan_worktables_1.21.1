...
import com.hikone.artisanworktables.common.recipe.ArtisanToolHandlers;
 ...
 */
public class ArtisanRecipeTransferHandler<C extends BaseContainer> implements IRecipeTransferHandler<C, ArtisanRecipe>
{
    ...
    public ArtisanRecipeTransferHandler(RecipeTransferInfo<C> transferInfo, IRecipeTransferHandlerHelper helper, IIngredientManager ingredientManager)
    {
    ...
    public IRecipeTransferError transferRecipe(C container, ArtisanRecipe recipe, IRecipeSlotsView recipeSlots, Player player, boolean maxTransfer, boolean doTransfer)
    {
        ...
        for (IRecipeSlotView slotView : recipeSlots.getSlotViews())
        {
            ...
            Optional<ITypedIngredient<ItemStack>> typed = this.ingredientManager.createTypedIngredient(VanillaTypes.ITEM_STACK, matched, false);
        ...
        {
            return this.helper.createUserErrorForMissingSlots(Component.translatable("jei.artisanworktables.transfer.missing_tool"), missingTools);
        }

        IRecipeSlotsView effectiveView = replaced ? this.helper.createRecipeSlotsView(adjustedViews) : recipeSlots;
    ...
    private ItemStack findInventoryTool(ToolEntry toolEntry, ArtisanRecipe recipe, List<Slot> inventorySlots, Set<Integer> usedSlots)
    {
        for (Slot slot : inventorySlots)
        {
            ...
            if (toolEntry.matches(handler, stack) && recipe.hasSufficientToolDurability(handler, stack))
            {
    ...
    private static final class ToolSlotView implements IRecipeSlotView
    {
