...
{
    public static int getPlayerExperienceTotal(Player player)
    {
        ...
        return (int) (experienceFromLevel + (player.experienceProgress * experienceBarCapacity));
    }

    public static void adjustPlayerExperience(Player player, int amount)
    {
        int experience = EnchantmentHelper.getPlayerExperienceTotal(player);
        player.totalExperience = Math.max(0, experience + amount);
        player.experienceLevel = EnchantmentHelper.getLevelFromExperience(player.totalExperience);
        player.experienceProgress = (player.totalExperience - EnchantmentHelper.getExperienceFromLevel(player.experienceLevel));
        player.experienceProgress /= (float) EnchantmentHelper.getExperienceBarCapacity(player.experienceLevel);
    }
