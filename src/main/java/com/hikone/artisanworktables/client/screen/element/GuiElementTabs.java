...
public class GuiElementTabs extends GuiElementBase implements IGuiElementClickable, IGuiElementTooltipProvider
{
    ...
    public GuiElementTabs(GuiContainerBase guiBase, BaseBlockEntity worktable, int elementWidth)
    {
    ...
    public void drawBackgroundLayer(PoseStack matrixStack, float partialTicks, int mouseX, int mouseY)
    {
        ...
        List<BaseBlockEntity> actualJoinedTables = this.worktable.getJoinedTables(new ArrayList<>(), Minecraft.getInstance().player, BaseBlockEntity::allowTabs);
        List<BaseBlockEntity> joinedTables = this.getJoinedTableOffsetView(actualJoinedTables, GUI_TAB_OFFSET.getOffset());
        ...
        if (GUI_TAB_OFFSET.getOffset() > 0)
        {
            ...
            GuiHelper.drawModalRectWithCustomSizedTexture(matrixStack, this.elementXModifiedGet() + TAB_LEFT_OFFSET + TAB_ITEM_HORIZONTAL_OFFSET - 18, tabY, 0, TAB_WIDTH + textureX, (this.worktable.getWorktableGuiTabTextureYOffset() % 12) * TAB_HEIGHT, BUTTON_WIDTH, TAB_HEIGHT, ReferenceTexture.TEXTURE_GUI_ELEMENTS_WIDTH, ReferenceTexture.TEXTURE_GUI_ELEMENTS_HEIGHT);
        }
        ...
        if (GUI_TAB_OFFSET.getOffset() + MAX_TAB_COUNT < actualJoinedTables.size())
        {
            ...
            GuiHelper.drawModalRectWithCustomSizedTexture(matrixStack, this.elementXModifiedGet() + this.elementWidthModifiedGet() - 12, tabY, 0, TAB_WIDTH + BUTTON_WIDTH + textureX, (this.worktable.getWorktableGuiTabTextureYOffset() % 12) * TAB_HEIGHT, BUTTON_WIDTH, TAB_HEIGHT, ReferenceTexture.TEXTURE_GUI_ELEMENTS_WIDTH, ReferenceTexture.TEXTURE_GUI_ELEMENTS_HEIGHT);
        }
        ...
        for (BaseBlockEntity joinedTable : joinedTables)
        {
            ...
            {
                GuiHelper.drawModalRectWithCustomSizedTexture(matrixStack, tabX, tabY + TAB_CURRENT_OFFSET, 0, textureX, textureY, TAB_WIDTH, TAB_HEIGHT, ReferenceTexture.TEXTURE_GUI_ELEMENTS_WIDTH, ReferenceTexture.TEXTURE_GUI_ELEMENTS_HEIGHT);
            }
            else
            {
                GuiHelper.drawModalRectWithCustomSizedTexture(matrixStack, tabX, tabY, 0, textureX, textureY, TAB_WIDTH, TAB_HEIGHT, ReferenceTexture.TEXTURE_GUI_ELEMENTS_WIDTH, ReferenceTexture.TEXTURE_GUI_ELEMENTS_HEIGHT);
            }
    ...
    public void mouseClicked(double mouseX, double mouseY, int mouseButton)
    {
        ...
        for (int i = 0; i < joinedTables.size(); i++)
        {
            ...
            if (mouseX <= xMax && mouseX >= xMin && mouseY <= yMax && mouseY >= yMin)
            {
                ...
                ArtisanWorktablesMod.getProxy().getPacketService().sendToServer(new CSPacketWorktableTab(pos, mouseX, mouseY));
    ...
    private void calculateInitialTabOffset(BaseBlockEntity worktableBase, GuiTabOffset guiTabOffset)
    {
        ...
        List<BaseBlockEntity> actualJoinedTables = worktableBase.getJoinedTables(new ArrayList<>(), Minecraft.getInstance().player, BaseBlockEntity::allowTabs);

        boolean tabInView = false;

        while (!tabInView && !actualJoinedTables.isEmpty())
        {
            List<BaseBlockEntity> joinedTables = this.getJoinedTableOffsetView(actualJoinedTables, guiTabOffset.getOffset());
            ...
            if (!tabInView)
            {
                guiTabOffset.setOffset(Math.min(actualJoinedTables.size() - MAX_TAB_COUNT, guiTabOffset.getOffset() + MAX_TAB_COUNT));
            }
        }
    }

    private List<BaseBlockEntity> getJoinedTableOffsetView(List<BaseBlockEntity> list, int offset)
    {
